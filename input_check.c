...
/*   Updated: 2023/12/15 18:05:23 by fstark           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int check_if_allowed(int new, int last) 
{
	if (last == 3)
	{
		if (new != 1 && new != 2)
...
int	ft_check_input_syntax(t_info *info)
{
	...
	while (info->input[i] != '\0')
	{
		if (info->input[i] == ' ' || info->input[i] == '\t')
			i++;
		if (ft_strchr2("><|", info->input[i]) && stateSingle == 0 && stateDouble == 0)
		{
			if (info->input[i] == '>' && info->input[i + 1] == '>')
				i++;
			else if (info->input[i] == '<' && info->input[i + 1] == '<')
			...
			if (info->input[i] == '|')
			{
				if (check_if_allowed(3, type_last) == 1)
					return (1);
				type_last = 3;
			}
			else
			{
				if (check_if_allowed(2, type_last) == 1)
					return (1);
				type_last = 2;
			}
		}
		...
		{
			i = handle_prompt_syntax(info, i);
			if (check_if_allowed(1, type_last) == 1)
				return (1);
			type_last = 1;
		}
		i++;
